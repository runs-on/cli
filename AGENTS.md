...
- Read `../docs/PROJECT_CONTEXT.md` first for product/business context and the list of customer-facing invariants.
- CLI changes can affect a documented public contract. Check the referenced public docs before changing commands, flags, install wording, troubleshooting guidance, or other user-visible behavior.
...
### Release Process
- `make sync-metadata` - Update release-facing version references in `README.md` and `action.yml`
- CLI tags, binaries, and GitHub releases are published from the monorepo root via `releasectl release final`
